...
if [ -z "$TRAVIS_PULL_REQUEST" ] || [ "$TRAVIS_PULL_REQUEST" == "false" ]; then
  ...
  if [ "$TRAVIS_BRANCH" == "master" ]; then
    ...
    $TRAVIS_BUILD_DIR/ecs-cli compose service up --target-group-arn arn:aws:elasticloadbalancing:us-east-1:377024778620:targetgroup/gateway/1c51fea8fd2329be --container-name gateway --container-port 8081 --role gateway-dev-us-east-1-ecs-service
  else
